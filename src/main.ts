...
interface TapePattern {
	id: string;
	label: string;
	/** Solid base color behind the pattern (function = resolved at use time).
	 * Doubles as a fallback fill if the CSS layer doesn't render. */
	base?: string | (() => string);
	/** SVG <pattern> definition (accent grid); angle = tape rotation in degrees. */
	defs?: (pid: string, angle: number) => string;
	/** Inline CSS background declaration(s), rendered verbatim via <foreignObject>
	 * — lets a pattern use conic-gradient etc. Takes precedence over `defs`. */
	css?: string;
	/** Smaller-scale CSS for the toolbar swatch (falls back to `css`). */
	swatch?: string;
}

/** Swatch background for the sub-bar icon — the pattern's own CSS (scaled), or
 * an SVG-pattern preview for `defs`-based patterns. */
function tapeSwatchCss(p: TapePattern): string {
	if (p.css) return p.swatch ?? p.css;
	const base = typeof p.base === "function" ? p.base() : (p.base ?? "#ffffff");
	const svg =
		`<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22">` +
		`<defs>${p.defs ? p.defs("sw", 0) : ""}</defs>` +
...
const TAPE_PATTERNS: TapePattern[] = [
	...
	},
	{
		...
		base: "#c8c96b",
		css:
			"--s:30px;--c1:#c8c96b;--c2:#fea0bb;background:" +
		...
		swatch:
			"--s:6px;--c1:#c8c96b;--c2:#fea0bb;background:" +
			"conic-gradient(at 60% 60%,var(--c1) 75%,#0000 0) 0 0/calc(5*var(--s)/2) calc(5*var(--s)/2)," +
			"repeating-conic-gradient(var(--c1) 0 25%,#0000 0 50%) 0 0/calc(5*var(--s)) calc(5*var(--s))," +
			"repeating-conic-gradient(var(--c2) 0 25%,var(--c1) 0 50%) 0 0/var(--s) var(--s);",
	},
	...
	},
	{
		...
		css:
			...
			"conic-gradient(transparent 25%,hsl(50,60%,65%) 0 50%,transparent 0 75%,#000 0) 0/4em 4em," +
			"repeating-conic-gradient(hsl(50,60%,65%) 0 25%,#000 0 50%) 0/.5em .5em;",
		swatch:
			"font-size:6px;background:" +
			"conic-gradient(transparent 25%,hsl(50,60%,65%) 0 50%,transparent 0 75%,#000 0) 0/4em 4em," +
			"repeating-conic-gradient(hsl(50,60%,65%) 0 25%,#000 0 50%) 0/.5em .5em;",
	},
];
...
// Marquee icons (designed, filled — dashed corner-marks rectangle + freehand
// lasso loop made of dashes).
const ICON_MARQUEE_RECT = filledMulti(
	"M1.03284 20.5038V19.4062C1.03285 18.992 1.36863 18.6562 1.78284 18.6562C2.19704 18.6562 2.53282 18.992 2.53284 19.4062V20.5038C2.53294 20.6418 2.64483 20.7538 2.78284 20.7538H3.72522C4.13941 20.7538 4.47522 21.0896 4.47522 21.5038C4.47512 21.9179 4.13935 22.2538 3.72522 22.2538H2.78284C1.8164 22.2538 1.03294 21.4702 1.03284 20.5038ZM8.43713 20.7538C8.85121 20.754 9.18713 21.0897 9.18713 21.5038C9.18703 
21.8795 5.13956V4.0419C21.8795 3.90383 21.7676 3.7919 21.6295 3.7919H20.6871C20.2729 3.79187 19.9371 3.4561 19.9371 3.0419C19.9371 2.6277 20.2729 2.29193 20.6871 2.2919H21.6295C22.596 2.2919 23.3795 3.0754 23.3795 4.0419V5.13956C23.3794 5.55368 23.0437 5.88956 22.6295 5.88956C22.2154 5.88956 21.8796 5.55368 21.8795 5.13956ZM8.43713 2.2919C8.85121 2.29206 9.18713 2.62778 9.18713 3.0419C9.18713 3.45602 8.85121 3.79174 8.43713 3.7919H6.55237C6.13815 3.7919 5.80237 3.45611 5.80237 3.0419C5.80237 2.62769 6.13815 2.2919 6.55237 2.2919H8.43713ZM13.1481 2.2919C13.5623 2.2919 13.8981 2.62769 13.8981 3.0419C13.8981 3.45611 13.5623 3.7919 13.1481 3.7919H11.2643C10.8501 3.7919 10.5143 3.45611 10.5143 3.0419C10.5143 2.62769 10.8501 2.2919 11.2643 2.2919H13.1481ZM17.86 2.2919C18.2742 2.2919 18.61 2.62769 18.61 3.0419C18.61 3.45611 18.2742 3.7919 17.86 3.7919H15.9752C15.5611 3.79174 15.2252 3.45602 15.2252 3.0419C15.2252 2.62778 15.5611 2.29206 15.9752 2.2919H17.86Z"
);
const ICON_LASSO = filledMulti(
...
class CanvasToolbar {
	...
	/** What a node contributes to search. */
	...
	refreshNodeStyles() {
		...
		for (const node of canvas.nodes.values()) {
			...
			if (typeof text === "string" && text.startsWith("<svg") && text.includes(INK_MARK)) {
				...
				blockDblClick(el); // never open Obsidian's raw-SVG editor
...
/** One captured stroke as parallel coordinate arrays. */
interface InkStroke {
	x: number[];
	y: number[];
}
...
function buildTapeSvg(
	...
	// CSS-background patterns (conic-gradients etc.): the tape SVG mounts inline,
	// so Chromium renders the CSS verbatim inside a <foreignObject>. A base-color
	// fill sits behind as a fallback; both are clipped to the torn edge and the
	// content is rotated to the drag direction.
	const cssPat =
		patternId !== CUSTOM_TAPE_ID
			? TAPE_PATTERNS.find((p) => p.id === patternId)
			: undefined;
	if (cssPat?.css) {
		const half2 = thickness / 2;
		const fb = typeof cssPat.base === "function" ? cssPat.base() : (cssPat.base ?? "#fff");
		const style = `width:${r2(len)}px;height:${thickness}px;${cssPat.css}`;
		const svg =
			...
			`<foreignObject x="0" y="${r2(-half2)}" width="${r2(len)}" height="${thickness}">` +
	...
	if (patternId === CUSTOM_TAPE_ID && customImage) {
		...
		const pattern =
			TAPE_PATTERNS.find((p) => p.id === patternId && p.defs) ?? TAPE_PATTERNS[0];
		base = typeof pattern.base === "function" ? pattern.base() : (pattern.base ?? "#fff");
		defs = pattern.defs ? pattern.defs(pid, Number(angle)) : "";
	}
